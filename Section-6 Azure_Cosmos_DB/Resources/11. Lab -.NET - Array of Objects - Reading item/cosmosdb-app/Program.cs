...
static async Task ReadItem(CosmosClient client)
{
    ...
    Console.WriteLine($"Order: {order.Id} | {order.CustomerName} | {order.Status}");
        Console.WriteLine($"Course: {order.Course.CourseId} | {order.Course.CourseName} | {order.Course.Category}");
        Console.WriteLine($"Payment: {order.Payment.Method} | {order.Payment.Provider} | {order.Payment.PaidAmount} {order.Payment.Currency}");
}
